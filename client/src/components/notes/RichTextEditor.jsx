...
import {
  Bold, Italic, Underline as ULine, Strikethrough,
  Heading1, Heading2, Heading3, List, ListOrdered, CheckSquare,
  Quote, Code, Link as LinkIcon, AlignLeft, AlignCenter, AlignRight,
  Highlighter, Minus, Undo2, Redo2, Code2
} from 'lucide-react'
...
const TB = ({ onClick, active, disabled, title, children }) => (
  <button onMouseDown={e => { e.preventDefault(); onClick() }} disabled={disabled} title={title}
    className={`p-1.5 rounded text-sm transition-colors ${active ? 'bg-ink-900 text-parchment-50' : 'text-ink-600 hover:bg-ink-100 hover:text-ink-900'} disabled:opacity-30 disabled:cursor-not-allowed`}>
    {children}
  </button>
)

const Sep = () => <div className="w-px h-5 bg-ink-200 mx-0.5" />
    ...
    <div className="flex flex-col h-full">
      {editable && (
        <div className="flex flex-wrap items-center gap-0.5 px-4 py-2 border-b border-ink-100 bg-parchment-50/80 sticky top-0 z-10 backdrop-blur-sm">
          <TB onClick={() => editor.chain().focus().undo().run()} disabled={!editor.can().undo()} title="Undo"><Undo2 size={14} /></TB>
          <TB onClick={() => editor.chain().focus().redo().run()} disabled={!editor.can().redo()} title="Redo"><Redo2 size={14} /></TB>
          <Sep />
          ...
          <TB onClick={() => editor.chain().focus().toggleUnderline().run()} active={editor.isActive('underline')} title="Underline"><ULine size={14} /></TB>
          ...
          <TB onClick={() => editor.chain().focus().toggleHighlight().run()} active={editor.isActive('highlight')} title="Highlight"><Highlighter size={14} /></TB>
          <TB onClick={() => editor.chain().focus().toggleCode().run()} active={editor.isActive('code')} title="Code"><Code size={14} /></TB>
          <TB onClick={setLink} active={editor.isActive('link')} title="Link"><LinkIcon size={14} /></TB>
          <Sep />
          ...
          <TB onClick={() => editor.chain().focus().toggleCodeBlock().run()} active={editor.isActive('codeBlock')} title="Code block"><Code2 size={14} /></TB>
          ...
          <div className="ml-auto flex items-center gap-3 text-xs text-ink-400 font-mono">
      ...
      <div className="flex-1 overflow-y-auto">
        <EditorContent editor={editor} className="h-full px-6 py-5" />
      </div>
    </div>
  )
}