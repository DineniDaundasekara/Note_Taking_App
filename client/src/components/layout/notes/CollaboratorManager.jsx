...
export default function CollaboratorManager({ note, onClose }) {
  ...
  const handleRemove = async (userId, name) => {
    if (!confirm(`Remove ${name} from collaborators?`)) return
    try { await removeCollaborator(note._id, userId); toast.success('Collaborator removed') }
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
      <div className="bg-white rounded-2xl shadow-modal w-full max-w-md animate-scale-in">
        <div className="flex items-center justify-between p-5 border-b border-ink-100">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 bg-ink-100 rounded-lg flex items-center justify-center">
              <Users size={16} className="text-ink-700" />
            </div>
            <div>
              <h2 className="font-display font-semibold text-ink-950">Collaborators</h2>
              <p className="text-xs text-ink-500 mt-0.5 truncate max-w-[200px]">{note.title}</p>
            </div>
          </div>
        ...
        <div className="p-5 space-y-5">
          {isOwner && (
            <div>
              <label className="block text-xs font-medium text-ink-700 mb-2 uppercase tracking-wider">Add person</label>
              <form onSubmit={handleAdd} className="space-y-3">
                <div className="relative">
                  <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-ink-400" />
                  <input type="email" className="input-field pl-9" placeholder="Search by email…"
                    value={email} onChange={e => handleSearch(e.target.value)} />
                  {searchResults.length > 0 && (
                    <div className="absolute top-full left-0 right-0 bg-white border border-ink-200 rounded-lg shadow-modal mt-1 z-10 overflow-hidden">
                      {searchResults.map(u => (
                        <button key={u._id} type="button" onClick={() => { setEmail(u.email); setSearchResults([]) }}
                ...
                <div className="flex gap-2">
                  <select className="input-field flex-1" value={permission} onChange={e => setPermission(e.target.value)}>
          ...
          <div>
            <label className="block text-xs font-medium text-ink-700 mb-2 uppercase tracking-wider">
              People with access ({note.collaborators.length + 1})
            </label>
            <div className="space-y-2">
              <div className="flex items-center gap-3 p-2.5 rounded-lg bg-parchment-50 border border-parchment-200">
                ...
                <div className="flex items-center gap-1 text-xs text-parchment-600 font-medium">
                  <Crown size={12} /> Owner
                </div>
              </div>

              {note.collaborators.map(c => (
                <div key={c.user._id} className="flex items-center gap-3 p-2.5 rounded-lg hover:bg-ink-50 group">
                  ...
                  {isOwner ? (
                    <div className="flex items-center gap-1">
                      <select value={c.permission} onChange={e => handleUpdatePerm(c.user._id, e.target.value)}
                        className="text-xs border border-ink-200 rounded px-1.5 py-1 text-ink-700 bg-white focus:outline-none">
                        <option value="read">View</option>
                        <option value="write">Edit</option>
                      </select>
                      <button onClick={() => handleRemove(c.user._id, c.user.name)}
                        className="p-1.5 text-ink-300 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100">
                        <Trash2 size={13} />
                      </button>
                    </div>
                  ) : (
